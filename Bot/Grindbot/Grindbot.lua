...
function Grindbot:Pulse()
    ...
    if not Throttle then
        ...
        self:DeleteTask()
        self:ClearblackList()
        if DMW.Player.PetActive and not Combat:SearchEnemy() then PetFollow() end -- Stupid ass rotations using pets unnecesary
        Throttle = true
    ...
    if LootUnit then
        if LootUnit.Distance >= 5 then
            ...
            if Navigation:ReturnPathEnd() ~= nil then
                ...
                if endPathToUnitDist > 2 or DMW.Time - moveToLootTime > 6 then
                    -- Blacklist unit
