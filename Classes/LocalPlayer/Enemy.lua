...
function LocalPlayer:GetEnemies(Yards)
    ...
    if DMW.Settings.profile.HUD.Mode and DMW.Settings.profile.HUD.Mode == 2 then
        if self.Target and self.Target.ValidEnemy and self.Target.Distance <= Yards and UnitCreatureTypeID(self.Target.Pointer) ~= 11 then
...
function LocalPlayer:GetHostiles(Yards)
    local Table = {}
    local Count = 0
